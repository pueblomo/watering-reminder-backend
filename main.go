...
func setRoutes(app *fiber.App) {
	...
	plantGroup.Delete("/", func(c *fiber.Ctx) error {
		return deletePlant(c)
	})
}
...
func deletePlant(ctx *fiber.Ctx) error {
	log.Println("Delete Plant")

	var plant Plant
	err := json.Unmarshal([]byte(ctx.FormValue("data")), &plant)
	if err != nil {
		log.Println(err)
		return ctx.SendStatus(fiber.StatusInternalServerError)
	}

	deleteImage(plant.ImageName)
	result := db.Delete(&plant)
	if result.Error != nil {
		log.Println(result.Error)
		return ctx.SendStatus(fiber.StatusInternalServerError)
	}
